...
require_once DAOS . '/RotasDao.class.php';
...
class Rotas  {
    ...
    public function save() {
        ...
        $nova_rota = $this->dao->save($this->input->cod_empresa, $this->input->id_unidade, $this->input->id_user, $this->input->nome, $this->data_atual, $status=1);   
              
        for($i = 0; $i<count($this->input->locais); $i++) {
           
            $this->dao->saveRotasLocais(
                $nova_rota, 
                $this->input->locais[$i]->id_local, 
                $this->input->locais[$i]->id_atividade                
            );
        }
        
        $result['status'] = '01';
        $result['msg'] = 'Rota adicionada';
