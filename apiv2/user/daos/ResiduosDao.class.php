<?php
  ...
  class ResiduosDao extends Conexao {
        ...
        public function deleteGrupo($id_grupo) {
          ...
          $sql_cadastro = $this->mysqli->prepare("DELETE FROM app_residuos_grupo WHERE id='$id'");
          $sql_cadastro->execute();
          ...
          public function saveSubgrupo($id_grupo, $nome, $data, $status) {
            ...
            $sql_cadastro = $this->mysqli->prepare("INSERT INTO `app_residuos_subgrupos`(`app_residuos_grupos_id`, `nome`, `data`, `status`)");
            $sql_cadastro->execute();
        ...
        public function listSubgrupos($id_grupo) {
          ...
          $sql->execute();
          $sql->bind_result($this->id, $this->nome, $this->grupo, $this->status, $this->id_grupo);
          $sql->store_result();
          $rows = $sql->num_rows;
    ...
    public function saveTipo($cod_empresa, $nome, $data, $status) {
  ...
  public function deleteTipo($id_tipo) {
    ...
    $sql_cadastro = $this->mysqli->prepare("DELETE FROM app_residuos_tipos WHERE id='$id'");
    $sql_cadastro->execute();
